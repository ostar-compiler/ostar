...
#include <ostar/relay/parser.h>
#include <ostar/relay/qnn/transform.h>
#include <ostar/runtime/ndarray.h>
#include <ostar/tir/stmt_functor.h>
...
OSTAR_REGISTER_NODE_TYPE(StorageInfoNode);
...
OSTAR_REGISTER_GLOBAL("relay.ir.StorageInfo")
...
OSTAR_REGISTER_GLOBAL("relay.ir.StorageInfoStorageIds").set_body_typed([](StorageInfo si) {
  Array<ostar::Integer> ids;
...
OSTAR_REGISTER_GLOBAL("relay.ir.StorageInfoDeviceTypes").set_body_typed([](StorageInfo si) {
  Array<ostar::Integer> device_types;
...
OSTAR_REGISTER_GLOBAL("relay.ir.StorageInfoStorageSizes").set_body_typed([](StorageInfo si) {
  Array<ostar::Integer> storage_sizes_in_bytes;
...
OSTAR_REGISTER_GLOBAL("relay.ir.StorageInfoVirtualDevices").set_body_typed([](StorageInfo si) {
...
OSTAR_REGISTER_NODE_TYPE(StaticMemoryPlanNode);
...
OSTAR_REGISTER_GLOBAL("relay.ir.StaticMemoryPlan")
...
OSTAR_REGISTER_NODE_TYPE(FunctionInfoNode);
...
OSTAR_REGISTER_NODE_TYPE(ExecutorCodegenMetadataNode);
...
Array<Pass> GetPassPrefix(bool is_homogeneous, bool is_vm) {
  ...
  PackedFunc fskip = PackedFunc([](OSTARArgs args, OSTARRetValue* rv) {
    Expr expr = args[0];
...
Map<Target, IRModule> TargetStrModuleMapToTargetModuleMap(
  ...
  Map<Target, IRModule> ostar_map;
  for (auto kv : input_map) {
    ostar_map.Set(kv.first, kv.second);
  }
  return ostar_map;
}
...
std::vector<int64_t> ShapeToJSON(ostar::Array<IndexExpr> shape) {
...
OSTAR_REGISTER_GLOBAL("relay.backend.tir_converter.default")
...
OSTAR_REGISTER_GLOBAL("relay.backend.tir_converter.allow_extern")
    .set_body_typed([](const Array<te::Tensor>& args,
                       const Array<runtime::NDArray>& constants) -> Optional<tir::PrimFunc> {
