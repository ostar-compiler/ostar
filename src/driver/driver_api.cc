...
#include <ostar/ir/transform.h>
#include <ostar/relay/executor.h>
#include <ostar/relay/runtime.h>
#include <ostar/runtime/registry.h>
#include <ostar/target/codegen.h>
#include <ostar/te/operation.h>
#include <ostar/tir/analysis.h>
#include <ostar/tir/transform.h>
...
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.noalias", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.detect_global_barrier", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.instrument_bound_checkers", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.disable_assert", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.disable_vectorize", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.disable_cse_tir", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.enable_debug", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.enable_equiv_terms_in_cse_tir", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.disable_storage_rewrite", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.is_entry_func", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.add_lower_pass", Array<Array<ObjectRef>>);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.debug_keep_trivial_loop", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.use_async_copy", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.instrument_lwp", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.vtcm_capacity", Integer);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.ptx_ldg32", Bool);
OSTAR_REGISTER_PASS_CONFIG_OPTION("tir.experimental_dma_bypass_cache", Bool);

using ostar::Array;
using ostar::transform::Pass;
...
bool ShouldAnnotateEntryFunc(const IRModule mod) {
  Optional<ostar::relay::Executor> executor = mod->GetAttr<ostar::relay::Executor>("executor");
...
OSTAR_REGISTER_GLOBAL("driver.get_binds")
    .set_body_typed([](const Array<ObjectRef>& args, bool compact,
      ...
      // OSTAR object system doesn't have a pair object, so we'll put both ret values in an array
      // and return that.
      Array<ObjectRef> out_arr = {out_binds, out_arg_list};
      return out_arr;
    });

Array<ostar::transform::Pass> CreatePassList(bool disable_loop_partition) {
  ...
  for (Array<ObjectRef> phase_pass : add_lower_pass) {
    ...
    auto pass = Downcast<ostar::transform::Pass>(phase_pass[1]);
  ...
  // PHASE 0
  Array<ostar::transform::Pass> pass_list = user_lower_phase0;
...
IRModule LowerWithPassList(IRModule mod, Array<ostar::transform::Pass> pass_list) {
  auto optimize = ostar::transform::Sequential(pass_list);
...
OSTAR_REGISTER_GLOBAL("driver.schedule_to_module")
...
OSTAR_REGISTER_GLOBAL("driver.lower_module").set_body_typed([](IRModule mod, bool simple_mode) {
...
OSTAR_REGISTER_GLOBAL("driver.lower_primfunc")
...
OSTAR_REGISTER_GLOBAL("driver.lower_schedule")
    .set_body_typed([](te::Schedule sch, const Array<ObjectRef>& args, const String& name,
...
OSTAR_REGISTER_GLOBAL("driver.tir_to_runtime")
...
transform::Sequential MixedModulePassManager(IRModule mixed_mod, Target target) {
  ...
  bool unpacked_api = mixed_mod->GetAttr<relay::Executor>(ostar::attr::kExecutor)
...
OSTAR_REGISTER_GLOBAL("driver.mixed_mod_passes")
...
transform::Sequential HostModulePassManager(IRModule mixed_mod, Target target_host) {
  ...
  Array<ostar::transform::Pass> host_pass_list;

  runtime::TypedPackedFunc<bool(tir::PrimFunc)> fcond = [](const tir::PrimFunc& f) {
    return f->GetAttr<Integer>(ostar::attr::kCallingConv, Integer(CallingConv::kDefault)) !=
  ...
  host_pass_list.push_back(tir::transform::LowerOSTARBuiltin());
...
OSTAR_REGISTER_GLOBAL("driver.host_mod_passes")
...
OSTAR_REGISTER_GLOBAL("driver.device_mod_passes")
    .set_body_typed([](IRModule mixed_mod, Target target_host) {
