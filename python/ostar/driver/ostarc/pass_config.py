...
def load_function(full_name):
    ...
    full_name: str
        ...
        OSTAR global function, if not find, try to import it by "importlib.import_module".
    Returns
    -------
    func: function or PackedFunc
        The loaded fucntion.
    """
    global_func = ostar.get_global_func(full_name, allow_missing=True)
    ...
    raise OSTARCException(f"No function '{func_name}' found in module '{module_name}'.")
...
def get_pass_config_value(name, value, config_type):
    ...
    if config_type == "IntImm":
        ...
        if parsed_value is None:
            raise OSTARCException(f"Invalid value '{value}' for configuration '{name}'.")
        ...
        if name == "tir.add_lower_pass":
            ...
            for i in range(0, len(pass_list), 2):
                ...
                try:
                    ...
                    raise OSTARCException(f"Only integer is allow for configuration '{name}'.")
            ...
            raise OSTARCException(f"Unsupported configuration '{name}' for '{config_type}' type.")
...
def parse_configs(input_configs):
    ...
    all_configs = ostar.ir.transform.PassContext.list_configs()
    ...
    for config in input_configs:
        ...
        if name not in all_configs:
            raise OSTARCException(
                f"Configuration '{name}' is not defined in OSTAR. "
                f"These are the existing configurations: {', '.join(all_configs)}"
            )

        if name not in supported_configs:
            raise OSTARCException(
                f"Configuration '{name}' uses a data type not supported by OSTARC. "
