...
def _load_lib():
    ...
    lib.OSTARGetLastError.restype = ctypes.c_char_p
    return lib, os.path.basename(lib_path[0])


try:
    # The following import is needed for OSTAR to work with pdb
...
# The FFI mode of OSTAR
_FFI_MODE = os.environ.get("OSTAR_FFI", "auto")
...
class OSTARError(RuntimeError):
    """Default error thrown by OSTAR functions.

    OSTARError will be raised if you do not give any error type specification,
    """
...
def register_error(func_name=None, cls=None):
    ...
    .. code-block:: python
      ...
      @ostar.error.register_error
      class MyError(RuntimeError):
          pass

      err_inst = ostar.error.create_ffi_error("MyError: xyz")
...
def get_last_ffi_error():
    c_err_msg = py_str(_LIB.OSTARGetLastError())
    py_err_msg, err_type = c2pyerror(c_err_msg)
    if err_type is not None and err_type.startswith("ostar.error."):
        err_type = err_type[10:]
    return ERROR_TYPE.get(err_type, OSTARError)(py_err_msg)
